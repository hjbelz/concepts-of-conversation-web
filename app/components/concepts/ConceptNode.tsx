...
const ConceptNode = ({ phenomenon = Phenomenon.Observable, isKeyConcept = false, icon = "Undefined", title, titleVariant, subtitle, children }: ConceptNodeProps): React.JSX.Element => {
  let stateSuffix = `${phenomenon}${isKeyConcept ? '--key-concept' : ''}`;
  let iconResource = (Icons[icon] !== undefined ? Icons[icon] : Icons["Undefined"]);
  return (
    <div className={`concept-node--${stateSuffix}`}>
      <div className="concept-node__header-box">
        {isKeyConcept && <img className="concept-node__icon" src={iconResource} alt={phenomenon} />}
